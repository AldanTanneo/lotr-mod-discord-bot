...
pub fn parse_web_colour(name: &str) -> Option<Colour> {
    WEB_COLOURS
        ...
        .ok()
        .map(|idx| Colour(WEB_COLOURS[idx].1))
}

#[cfg(test)]
#[test]
fn test_web_colour_search() {
    assert!(WEB_COLOURS.windows(2).all(|w| w[0].0 < w[1].0));

    for (s, c) in WEB_COLOURS {
        assert_eq!(parse_web_colour(s), Some(Colour(c)));
    }
}
